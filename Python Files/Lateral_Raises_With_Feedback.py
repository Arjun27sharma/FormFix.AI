...
with mp_pose.Pose(min_detection_confidence = 0.55, min_tracking_confidence = 0.55) as pose:
    ...
    #Detection and Rendering
        ...
        try:
            ...
            hip_elbow_angle = calculateAngle(hip, shoulder, elbow)
            elbow_wrist_angle = calculateAngle(shoulder, elbow, wrist)
            
            #Visualise the angle
            
#             cv2.putText(image, str(hip_elbow_angle),
#                           tuple(np.multiply(shoulder, [width,height]).astype(int)),
#                                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,255), 2, cv2.LINE_AA
#                         )
            cv2.putText(image, str(elbow_wrist_angle),
            ...
            if(counter != 0):
                if(hip_elbow_angle > 85):
                    cv2.putText(image, 'Too High, Keep your arms InLine to your shoulder', 
                                  tuple(np.multiply([0.6,0.2], [width,height]).astype(int)),
                                      cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,255), 2, cv2.LINE_AA)
                if((elbow_wrist_angle < 120 or elbow_wrist_angle > 160)):
                    cv2.putText(image, 'Keep your arms long with a Slight Bend in your Elbows and palms facing each other', 
                                  tuple(np.multiply([0.5,0.4], [width,height]).astype(int)),
                                      cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,255), 2, cv2.LINE_AA)
            if hip_elbow_angle < 10:
                stage = "Down"
            if hip_elbow_angle > 80 and stage == 'Down':
                stage = "Up"
                counter += 1
                #print(counter)
